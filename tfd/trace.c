...
void decode_address(uint32_t address, EntryHeader *eh, int ignore_taint)
{
  ...
  */
  ...
  if (tracepid != -1) {
    ...
    eh->pid = VMI_find_pid_by_cr3_c(DECAF_CPU_CR[3]);
  }
    ...
    switch(op_name) {
      ...
      case XED_OPERAND_MEM1: {
        ...
        for (j = 0; j < memlen; j+=4) {
          ...
          if ((eh->operand[op_idx].addr.mem32_addr < VMI_guest_kernel_base) &&
...
unsigned int write_insn(FILE *stream, EntryHeader *eh)
{
  ...
  if (header_already_written == 0) {
    ...
    tmodinfo_t *pmr =
      (tmodinfo_t *) malloc(pr.n_mods * sizeof(tmodinfo_t));
    if (pmr) {
...
int close_trace(FILE *stream) {
  ...
  if (proc_arr) {
    ...
    for (i = 0; i < num_procs; i++) {
      ...
      modlist_size = pr.n_mods * sizeof(tmodinfo_t);

      if (modlist_size > 0) {
        tmodinfo_t *pmr = (tmodinfo_t *) malloc(modlist_size);
        if (pmr) {
          VMI_get_proc_modules_c(pr.pid, pr.n_mods, pmr);
        }
        else {
